...
#include "Rotary.h"
#include <Wire.h> //Added code By Maarten Baeten
...
String analogtypeS[numberOfAnalogPins];//Added code By Maarten Baeten
int analogtypeI[numberOfAnalogPins];//Added code By Maarten Baeten
String inputs = ""; //Added code By Maarten Baeten
int k; //Added code By Maarten Baeten
...
void setup() {
  Wire.begin();        // join i2c bus (address optional for master) X
...
//        messagepayload[1] = messagepayload[1] - '0'; // I2C change
...
//      Serial.println(messagepayload[2] < digitaltype_t::NUMBEROFDIGITALTYPES);
      if (messagepayload[1] < numberOfDigitalPins && messagepayload[2] < digitaltype_t::NUMBEROFDIGITALTYPES) {
        //pinMode(messagepayload[1], INPUT); // I2C change
        ...
        Serial.println(messagepayload[1] < numberOfDigitalPins);
        Serial.println(messagepayload[2] < digitaltype_t::NUMBEROFDIGITALTYPES);
        Serial.println(messagepayload[1]);
...
void loop() {
  ...
  //----------------- Added code By Maarten Baeten  ----------------------------------
  Wire.requestFrom(8, numberOfDigitalPins+numberOfAnalogPins*4+2);    // request bytes from peripheral device #8
...
//  Serial.println("");
  ...
  for(int l=0;l<=k;l++){
    analogtypeI[l]=analogtypeS[l].toInt();
  }
  
  //Serial.println("end test"); 
 // Serial.println(analogtypeI[0]); 
//  Serial.println(analogtypeI[1]);
//  Serial.println(analogtypeI[2]);
//  Serial.println(analogtypeI[3]);
  inputs ="";
  //-------------------------------------------------
  ...
  for (int i = 0; i < numberOfDigitalPins; ++i) {
    ...
    if (digitaltype[i] == digitaltype_t::BUTTON) {
      int buttonState = buff[i] - '0'; //Added code By Maarten Baeten
      Serial.print(F("{DB"));
  ...
  for(int i = 0; i < numberOfAnalogPins; ++i){
    ...
    if(analogtype[i] == analogtype_t::SLIDER){
      int analogValue = analogtypeI[i];//Added code By Maarten Baeten
